...
import { getCrowndSaleAbi } from "./utils/getAbi";
import { getCrowndSaleAddress } from "./utils/getAddress";

export default class CrowndSaleContract extends BaseInterface {
    constructor(provider?: ethers.providers.Web3Provider) {
        const rpcProvider = new ethers.providers.JsonRpcProvider(getRPC())
        super(provider || rpcProvider, getCrowndSaleAddress(), getCrowndSaleAbi());
        if (!provider) {
